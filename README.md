# Seamus Lee

Welcome to my engineering project portfolio. This repository contains project reports covering automation, electronics, machine design, safety, mechanical components, and industrial problem-solving.

## About Me

I am developing hands-on engineering experience through individual projects, collaborative Capstone coursework, independent study, and machine design studies.

My areas of interest include PLC programming, Ladder Logic, Arduino and electronics, automation and control systems, mechanics of materials, mechanical design, safety, and technical problem-solving.

## Currently Learning

- PLC programming and Ladder Logic through independent study
- Collaborative engineering and project development through Capstone coursework
- Machine Design, including the application of mechanics of materials to mechanical components
- Teamwork, technical communication, and practical engineering problem-solving

## Project Reports

### Automated Pill Organizer

An automated mechanical system designed to organize and dispense pills. This project focuses on automation, mechanical design, user-focused problem-solving, and reliable operation.

**Technology and methods used:** Mechanical design, automation concepts, mechanical components, and technical documentation.

[View the Automated Pill Organizer Report](https://github.com/SeamusLee1/SeamusLee1/blob/989ba16b1ec5f91cd2153fddab040e1125380535/Automated%20Pill%20Organizer%20Report.pdf)

### Automated Sump Pump Control System

An automated control system designed to monitor sump conditions and control pump operation to help prevent flooding and improve system reliability.

**Technology and methods used:** Electrical controls, automation and control logic, monitoring systems, and technical documentation.

[View the Automated Sump Pump Control System Report](https://github.com/SeamusLee1/SeamusLee1/blob/989ba16b1ec5f91cd2153fddab040e1125380535/Automated%20Sump%20Pump%20Control%20System%20Report.pdf)

### Curious Arduino Box

An Arduino-based electronics project involving a microcontroller, programming, and interactive system design.

**Technology and methods used:** Arduino, embedded programming, electronics, input and output devices, and hands-on prototyping.

[View the Curious Arduino Box Report](https://github.com/SeamusLee1/SeamusLee1/blob/989ba16b1ec5f91cd2153fddab040e1125380535/Curious%20Arduino%20Box%20Report.pdf)

### Hair Care Heat-Safety Station

A safety-focused engineering project designed to address heat-related risks associated with hair-care equipment and activities.

**Technology and methods used:** Mechanical design, safety engineering, heat-management concepts, and technical documentation.

[View the Hair Care Heat-Safety Station Report](https://github.com/SeamusLee1/SeamusLee1/blob/989ba16b1ec5f91cd2153fddab040e1125380535/Hair%20Care%20Heat-Safety%20Station%20Report.pdf)

### Multi-Size Sheet Metal Can Bracket

A mechanical component designed to support cans of different sizes using sheet-metal construction.

**Technology and methods used:** Sheet-metal design, mechanical design, mechanics of materials, component design, and fabrication methods.

[View the Multi-Size Sheet Metal Can Bracket Report](https://github.com/SeamusLee1/SeamusLee1/blob/989ba16b1ec5f91cd2153fddab040e1125380535/Multi-Size%20Sheet%20Metal%20Can%20Bracket%20Report.pdf)

### Vial Wash Line Jam Detection System

An industrial automation project designed to detect jams in a vial washing line and support timely operator response.

**Technology and methods used:** Industrial automation, jam-detection systems, sensors, control logic, and process monitoring.

[View the Vial Wash Line Jam Detection System Report](https://github.com/SeamusLee1/SeamusLee1/blob/1863aa0efad5bc84d1b73e46a93c94faad420a5f/Vial%20Wash%20Line%20Jam%20Detection%20System%20Report.pdf)

## Engineering Interests

- PLC programming and Ladder Logic
- Industrial automation
- Arduino and embedded systems
- Machine Design
- Mechanics of materials
- Mechanical components
- Sheet-metal design
- Safety-focused engineering
- Process improvement
- Technical documentation

Thank you for visiting my engineering portfolio.
